...
// WRITE YOUR CODE HERE
			...
			for(count = 0; count < numberOfLines && StdIn.hasNextLine(); count++){
				...
				listOfPatients[count] = new Person(id,ethnicity,gender,age,cause,urgency,stateOfHealth);
				}
				return count;
			}
...
// WRITE YOUR CODE HERE
			...
			for(count = 0; count < numberOfLines && StdIn.hasNextLine(); count++){
				...
				survivabilityByAge[count] = new SurvivabilityByAge(age,years,rate);
			}
			return count;
		}
...
// WRITE YOUR CODE HERE
			...
			for(count = 0; count < numberOfLines && StdIn.hasNextLine(); count++){
				...
				survivabilityByCause[count] = new SurvivabilityByCause(cause,years,rate);
				}
			return count;
			}
...
// WRITE YOUR CODE HERE
			...
			Person[] ageAbove = new Person[count];
			for(int i=0,j=0;i<listOfPatients.length && j<count;i++) {
				if(listOfPatients[i].getAge()>age){
					ageAbove[j] = listOfPatients[i];
					j++;
					}
				}
				if(count == 0)
...
// WRITE YOUR CODE HERE
			...
			Person[] health = new Person[count];
			for(int i=0,j=0;i<listOfPatients.length && j<count;i++){
				if(listOfPatients[i].getStateOfHealth()==state){
					health[j] = listOfPatients[i];
					j++;
				}
			}
			if(count == 0)
...
// WRITE YOUR CODE HERE
			int count = 0;
				...
				Person[] condition = new Person[count];
				for(int i=0,j=0;i<listOfPatients.length && j<count;i++){
					if(listOfPatients[i].getCause()==cause){
						condition[j] = listOfPatients[i];
						j++;
					}
				}
				if(count == 0)
...
// WRITE YOUR CODE HERE
		if(numberOfHearts<listOfPatients.length){
			Person[] list = new Person[numberOfHearts];
			for(int i=0;i<numberOfHearts;i++){
				list[i] = listOfPatients[i];
			}
			return list;
		}
		else{
